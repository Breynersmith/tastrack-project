...
export default function Cards({ listId }) {
  ...
  // cambiar estado de targeta
  const handleStateCard = (cardId) => {
   dispatch(updateCard({cardId, updateField: {status : "completed"}}))
  }

  // Obtener tarjetas al cargar el componente
  useEffect(() => {
    dispatch(fetchCards(listId)); 
  }, [dispatch, listId]);
  ...
  return (
    <div className="p-1">
      {cards.length > 0 ? (
        cards.filter((card) => card.status === "in_progress").map((card, index) => (
              ...
              <div className="relative">
                ...
                {showMenuCard === index && (
                  <ul className="absolute right-6 -top-2 bg-gray-200 rounded-lg p-2 w-max z-10 text-center flex flex-col text-xs gap-4">
                    ...
                    onClick={() => handleStateCard(card.id)}>Finalizar Tarea</button></li>
                    <li className="py-1">
